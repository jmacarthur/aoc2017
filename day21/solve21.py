...
class Sprite(object):
    ...
    def direct_match_flip_v(self, pattern, x, y, size):
        for px in range(0,size):
            for py in range(0,size):
                if self.pixels[y+py][x+px] != pattern.pixels[size-1-py][px]:
                    return False
        logging.debug("Match!")
        return True
    ...
    def direct_match(self, pattern, x, y, size):
        for px in range(0,size):
            for py in range(0,size):
                if self.pixels[y+py][x+px] != pattern.pixels[py][px]:
                    return False
        logging.debug("Match!")
        return True
    def rotate_ccw(self):
        ...
        for row in range(0,self.size):
            for col in range(0,self.size):
                n.pixels[self.size-1-col][row] = self.pixels[row][col]
        self.pixels = n.pixels
...
def main():
    ...
    for i in range(0,18):
        logging.info("----- Iteration %d starts with:\n"%i)
